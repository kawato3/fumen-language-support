...
test('every bundled third-party component retains its license and attribution', () => {
  const notices = readFileSync('resources/vendor/THIRD_PARTY_NOTICES.md', 'utf8');
  const licenses = [
  ...
  for (const [filename, copyright] of licenses) {
    assert.ok(readFileSync(`resources/vendor/${filename}`, 'utf8').includes(copyright!), filename);
    assert.ok(notices.includes(`(${filename})`), `${filename} is discoverable in third-party notices`);
  }
  const bundle = readFileSync('resources/vendor/fumen.js', 'utf8');

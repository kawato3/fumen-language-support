...
- 公開物を作ったコミット `e674986` に、注釈付きタグ [`v1.0.1`](https://github.com/kawato3/fumen-language-support/releases/tag/v1.0.1) を付けました。
...
通常版・プレリリースを問わず、Marketplace に公開したら、その配布物を作ったコミットに SemVer の注釈付きタグ（例: `v1.2.3`）を必ず付けて GitHub にプッシュする。公開後の確認記録やドキュメントだけを追加したコミットではなく、VSIX と一致するコミットをタグの対象にする。

```sh
git tag -a v1.2.3 <release-commit> -m 'Release v1.2.3'
git push origin v1.2.3
```

タグをプッシュしたら、GitHub 上のタグが意図したコミットを指すことを確認する。`CHANGELOG.md` の版、`package.json` の版、Marketplace の公開版、タグ名は同じ番号に保つ。

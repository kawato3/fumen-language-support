...
別の Fumen 版へそのまま当てないでください。より新しい上流版で使う場合は、パッチをリベースしてからテストしてください。パッチにはソースコード、テスト、ビルド設定、文書を含め、生成物の `dist/` は含めていません。**適用後には必ずビルドしてください。** 上流の `dist/fumen.js` とソースマップは、再ビルドするまで元のままです。
...
`test:chord-labels` はテストの前に Webpack を実行し、`dist/fumen.js` と
`dist/fumen.js.map` の両方を再生成します。アプリケーションでは、このビルド済み
JavaScript を使用してください。ソースマップも配布する場合は、同時に生成されたものを使います。

テストでは、コンパクトな既定表示、両配置でのラベル変更、同じ意味の入力表記、移調、テンション、変化音、オンコード、小節番号を確認します。コード表示のテストは実際の Canvas への文字描画を検査するもので、ピクセル単位の浄書品質のテストではありません。

拡張バージョン 1.0.5 に同梱するレンダラーとビルド結果が一致するかは、次のコマンドで確認できます。

```sh
git hash-object dist/fumen.js
```

期待する Git blob ID は `8bfb817f8e9ad6c42f313d6be96e013e9819189b` です。
一致しない場合は、導入前に上流のコミット、パッチ、ロックファイルに従った依存関係を確認してください。
拡張に同梱するレンダラー自体の変更はありません。パッチの配布形式だけを、生成物を含める形式から
適用後にビルドする形式へ変更しています。
